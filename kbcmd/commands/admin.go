...
type CustomInvalidatesCacheByAccountNoContent struct {
	admin.InvalidatesCacheByAccountNoContent
	HttpResponseHandler
}

type CustomInvalidatesCacheByAccountBadRequest struct {
	admin.InvalidatesCacheByAccountBadRequest
	HttpResponseHandler
}
...
func invalidateAccountCache(ctx context.Context, o *cmdlib.Options) error {
	...
	autoGeneratedResp, apiErr := o.Client().Admin.InvalidatesCacheByAccount(ctx, &admin.InvalidatesCacheByAccountParams{
		AccountID: accountId,
	})

	// If there's an error from the API and autoGeneratedResp is nil, we handle the error directly.
	if apiErr != nil {
		killbillErr, ok := apiErr.(*kbcommon.KillbillError)
		if ok {
			// Handle the error using the KillbillError fields
			errorDetails := fmt.Sprintf("error: %s (HTTP Code: %d, Error Code: %d)", killbillErr.Message, killbillErr.HTTPCode, killbillErr.Code)

			return fmt.Errorf(errorDetails)
		}
		return apiErr // If it's not a KillbillError, just return the error as is
	}

	// If we're here, autoGeneratedResp is not nil.
	customResp := &CustomInvalidatesCacheByAccountNoContent{
		InvalidatesCacheByAccountNoContent: *autoGeneratedResp,
		HttpResponseHandler:                HttpResponseHandler{HttpResponse: autoGeneratedResp.HttpResponse},
	}

	errorMessage := ""
	if customResp.HttpResponseHandler.IsClientError() {
		errorMessage = "bad request when trying to invalidate account cache"
	} else if customResp.HttpResponseHandler.IsServerError() {
		errorMessage = "server error when trying to invalidate account cache"
	} else if !customResp.HttpResponseHandler.IsSuccess() {
		errorMessage = "failed to invalidate account cache"
	}

	if errorMessage != "" {
		errorDetails := fmt.Sprintf("%s: %d %s", errorMessage, customResp.HttpResponseHandler.HttpResponse.Code(), customResp.HttpResponseHandler.HttpResponse.Body())
